...
def pdf_to_images(pdf_path: str, dpi: int = 192) -> list[tuple[bytes, dict]]:
...
- **DPI 选择**：LLM 输入图片默认 192 DPI（2× 标准屏幕分辨率），足以让 GPT-5.4 清晰识别文字和布局细节，同时控制图片体积和 token 消耗；后处理阶段裁剪光栅图时仍使用 300 DPI 以保证输出质量
...
- 字体大小校准：注意不要高估字体大小，标准 16:9 slide 的典型字号范围见英文原文
...
p2p input.pdf -o output.pptx \
    ...
    --dpi 200 \                    # LLM 输入图片渲染 DPI（默认 192）
    --enable-animations \          # 启用动画 / 转场
    --reasoning-effort high \      # 推理强度：low/medium/high（默认 high）
...
完整转换同样将所有中间产物保存到带时间戳的 `run-` 前缀目录下：

```
run-example1-20260309-150000/
├── metadata.json                  # 运行元信息（含 token 估算、API 耗时等）
├── pages/
│   ├── page_000.png
│   └── ...
├── slides/
│   ├── slide_000.xml              # GPT-5.4 生成的每页 Slide XML
│   └── ...
...
对于分批处理的大文档，`api_response`、`stream_chunks`、`tool_calls` 文件会带批次后缀（如 `api_response_1.json`）。
...
[14:30:52] INFO     main           Options: dpi=192, animations=off, model=gpt-5.4
[14:30:52] INFO     preprocessor   Rendering PDF pages at 192 DPI...
[14:30:53] INFO     preprocessor     Page  0/20: 2667×2000 px, 480 KB
[14:30:53] INFO     preprocessor     Page  1/20: 2667×2000 px, 360 KB
...
[14:30:56] INFO     preprocessor   All 20 pages rendered in 4.0s (total 8.2 MB)
...
- 对于 192 DPI 的图片，1 像素 ≈ 0.375 pt ≈ 4763 EMU，这对于 LLM 的视觉分析提供了良好的精度
...
│   ├── api_client.py              # GPT-5.4 API 调用封装（流式输出）
...
├── tests/
│   └── test_e2e.py                # 端到端测试（含 Mock OpenAI 服务器）
