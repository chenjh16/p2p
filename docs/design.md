...
> **`ApiConfig` 数据类**：`api_key`、`api_base_url`、`model_name` 三个参数封装在 `src.ApiConfig`（frozen dataclass）中，由 `main.py` 统一解析后作为单一参数传递给 `replay.py`、`continue_run.py`、`api_client.py`、`api_client_anthropic.py`，避免多处重复获取环境变量。
