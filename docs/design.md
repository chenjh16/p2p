...
构建一个基于多模态大模型 GPT-5.4 的工具，将 PDF 格式的演示文稿（slides）转换为高保真、可编辑的 Microsoft PowerPoint（PPTX）文件。核心设计原则：
...
- **直接输出 Slide XML**：GPT-5.4 直接输出符合 Microsoft PowerPoint 标准的 PresentationML（OOXML）格式 slide XML，而非中间 JSON 指令，减少转换层次，最大化表达能力
...
| LLM | GPT-5.4 (OpenAI API) | 多模态分析 + 直接生成 Slide XML |
...
```
┌─────────────┐     ┌──────────────────┐     ┌───────────────────────┐     ┌──────────────────┐
│  PDF Input   │────▶│  Preprocessing   │────▶│  GPT-5.4 Generation   │────▶│  PPTX Assembler   │
...
def pdf_to_images(pdf_path: str, dpi: int = 300) -> list[tuple[bytes, dict]]:
    """将 PDF 每页转为高分辨率 PNG 图片。"""
...
- **DPI 选择**：默认 300 DPI，确保 GPT-5.4 能识别小字体和细节元素
...
def build_messages(
    ...
    if not enable_animations:
        system_prompt += (
            "\n\n## Animation Disabled\n"
            "Animation is disabled for this conversion. "
            "Do NOT include <p:transition> or <p:timing> elements in the slide XML."
        )
    messages = [
...
def call_gpt54(
    ...
    """单次 API 调用，GPT-5.4 通过 tool calling 输出每页的 slide XML。"""
    client_kwargs = {}
    if api_base_url:
    ...
    response = client.chat.completions.create(
        ...
        parallel_tool_calls=True,  # 允许并行调用，一次性输出所有页
        max_tokens=128000,
    )

    slide_xmls = {}
    for tool_call in response.choices[0].message.tool_calls:
        if tool_call.function.name == "write_slide_xml":
            args = json.loads(tool_call.function.arguments)
            slide_xmls[args["page_num"]] = args["slide_xml"]
...
```
你是一个专业的演示文稿分析与重建引擎。你的任务是分析 PDF slides 的图片，并为每页直接生成
符合 Microsoft PowerPoint 标准的 PresentationML（OOXML）格式 slide XML，使得重建后的
PPTX 文件在视觉上尽可能还原原始 slides，同时保持良好的可编辑性。输出必须完全兼容
Microsoft PowerPoint——所有元素都应能在 PowerPoint 中正常打开、编辑和演示。

## 输出方式

为 PDF 中的每一页调用一次 `write_slide_xml` 工具，传入：
- `page_num`：该页在 PDF 中的页码（0-indexed，与用户消息中的 Page 编号一致）
...
2. **光栅图占位**：当某个区域是无法用 Microsoft PowerPoint 内置编辑工具准确复现的光栅图像
...
[
  {
    "role": "system",
    "content": "<系统提示词（见第 6 节）>"
  },
  {
    "role": "user",
    "content": [
      { "type": "text", "text": "Analyze the following 20 slide pages... Slide dimensions: 960pt × 540pt..." },
...
GPT-5.4 的响应将包含 20 个 `write_slide_xml` 工具调用（`parallel_tool_calls=True`），每个调用携带一页的完整 slide XML。
...
python -m src.main input.pdf -o output.pptx

# 启用动画增强
python -m src.main input.pdf -o output.pptx --enable-animations
...
# 完整选项
python -m src.main input.pdf -o output.pptx \
    --api-base-url URL \           # OpenAI API Base URL（默认 $OPENAI_BASE_URL）
    --api-key KEY \                # OpenAI API Key（默认 $OPENAI_API_KEY）
    --model-name MODEL \           # 模型名称（默认 $OPENAI_MODEL_NAME 或 gpt-5.4）
    --dpi 400 \                    # 渲染 DPI（默认 300）
    --enable-animations \          # 启用动画 / 转场
    --batch-size 25 \              # 分批大小（默认 25）
...
| 参数 | 环境变量 | 默认值 | 说明 |
|------|----------|--------|------|
| `--api-base-url` | `OPENAI_BASE_URL` | `""` | OpenAI API Base URL |
...
```
dry-run-20260309-143052/
├── metadata.json                  # 运行元信息 + token 估算
├── pages/
...
### 9.4 metadata.json 示例

```json
{
  "timestamp": "2026-03-09T14:30:52Z",
  ...
  "token_estimate": {
    ...
    "image_count": 20,
    "image_tokens": 42500,
    "total_input_tokens": 46000,
    "estimated_output_tokens": 62000,
    "estimated_total_tokens": 108000,
    "estimated_cost_usd": {
      "input_cost_usd": 0.115,
      "output_cost_usd": 0.62,
      "total_cost_usd": 0.735
    }
  }
}
```
...
[14:30:52] INFO     main           Input: presentation.pdf (20 pages)
[14:30:52] INFO     main           Options: dpi=300, animations=off, model=gpt-5.4
[14:30:52] INFO     preprocessor   Rendering PDF pages at 300 DPI...
[14:30:53] INFO     preprocessor     Page  0/20: 4167×3125 px, 1.2 MB
[14:30:53] INFO     preprocessor     Page  1/20: 4167×3125 px, 0.8 MB
...
[14:30:58] INFO     preprocessor   All 20 pages rendered in 6.1s (total 18.4 MB)
[14:30:59] INFO     msg_builder    Building messages for 20 pages...
[14:30:59] INFO     msg_builder    Messages assembled: 41 content parts
[14:30:59] INFO     token_est      Token estimate: 46,000 input + ~62,000 output = ~108,000 total
[14:30:59] INFO     token_est      Estimated cost: $0.735
[14:30:59] INFO     api_client     Calling GPT-5.4 API (tool_calling, parallel, max_tokens=128000)...
...
1. **XML 解析验证**：使用 `lxml.etree.fromstring()` 解析，捕获 `XMLSyntaxError`
2. **自动修复**：对常见问题（未闭合标签、命名空间缺失）进行自动修复
3. **回退策略**：如果某页 XML 无法修复，记录错误并生成一个包含错误信息的占位 slide
...
- 对于 300 DPI 的图片，1 像素 ≈ 0.24 pt ≈ 3048 EMU，这提供了足够的精度
...
│   ├── api_client.py              # GPT-5.4 API 调用封装
│   ├── pptx_assembler.py          # Slide XML → PPTX 组装
...
4. **流式输出**：利用 OpenAI Streaming API，边接收 tool call 边组装 PPTX，降低用户等待时间
