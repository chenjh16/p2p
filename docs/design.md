...
- **图标矢量化**：简单图标（对勾、齿轮、灯泡等）使用 DrawingML 矢量绘制（预设形状、Unicode、自定义几何、组合形状），而非光栅占位符
...
- **TUI 确认**：每次 LLM API 调用前，使用 Rich TUI 面板展示批次信息（模型、Token、费用、时间），用户确认后执行；支持 `-y`/`--yes` 自动确认
- **Markdown 提示词管理**：系统提示词以 Markdown 文件形式存放于 `src/prompts/`，运行时加载，便于编辑和国际化
- **丰富日志**：全流程结构化日志输出，涵盖每个阶段的关键指标和进度信息
- **图片文件夹输入**：除 PDF 外，支持传入幻灯片截图文件夹（PNG、JPG 等），按文件名排序
- **错误恢复**：API 调用失败时交互式重试/跳过/退出，保留已完成进度
- **继续运行**：通过 `--continue-run` 从产物目录恢复未完成的转换
- **重放**：通过 `--replay` 使用相同参数重新执行之前的运行
- **供应商配置**：`.p2p.config` JSON 配置 per-provider 的 Responses API、额外请求头等
- **OpenAI Responses API**：可选的 `client.responses.create()` 路径，由 `.p2p.config` 或 `--use-responses-api` 启用
- **页面选择**：`--pages` 指定页面（如 `0,2,5-8`），`--max-pages` 限制最大页数
...
| OpenAI SDK | openai | OpenAI API 调用（含 Responses API） |
| Anthropic SDK | anthropic | Anthropic API 调用 |
| TUI | Rich | API 调用确认面板、批次信息展示 |
...
**图片文件夹输入**：除 PDF 文件外，工具还支持直接传入一个包含幻灯片截图的文件夹。文件夹中的图片文件（PNG、JPG、JPEG、BMP、TIFF、WebP）按文件名升序排列，每张图片作为一页处理。图片尺寸按 96 DPI 转换为 pt 坐标（1 px = 0.75 pt）。使用文件夹输入时，光栅后处理自动跳过（无源 PDF 可裁剪）。
...
def call_llm(
    ...
    if reasoning_effort:
        ...
        create_kwargs["max_completion_tokens"] = max_tokens  # reasoning models
    else:
...
**即时幻灯片持久化**：在流式接收过程中，每个 `write_slide_xml` 工具调用完成后，对应的 slide XML 会通过 `on_slide_ready` 回调立即写入磁盘（`slides/slide_NNN.xml`）。这确保即使在流式传输中途崩溃，已完成的幻灯片也不会丢失。

**跨批次页码映射**：每个批次在与 LLM 通信时使用批次内部编号（0 到 batch_size-1），通过 `batch_page_map` 映射表将 LLM 返回的批次内部页码转换为实际 PDF 页码。此映射同时应用于 `on_slide_ready` 回调和 `result.slide_xmls` 字典，确保 `slides/` 目录下的 XML 文件使用正确的实际页码命名。

**Anthropic API 调用（Claude Opus/Sonnet 4.6）：**

对于 Opus 4.6 / Sonnet 4.6，使用 **adaptive thinking** 替代已弃用的 `thinking.type=enabled` + `budget_tokens`：
...
# 旧模型 (Opus 4.5, Sonnet 4.5 等): 仍使用 budget_tokens
create_kwargs["thinking"] = {"type": "enabled", "budget_tokens": thinking_budget}
```

CLI `--reasoning-effort` 映射到 Anthropic `effort` 参数：

| CLI 参数 | Anthropic effort | 说明 |
|---------|-----------------|------|
| `low` | `low` | 最高效，简单任务可跳过思考 |
| `medium` | `medium` | 平衡速度与性能 |
| `high` | `high`（默认） | 深度推理 |
| `xhigh` | `max`（仅 Opus 4.6） | 无约束最大能力 |

**超时配置**：HTTP 超时使用 `httpx.Timeout(timeout_seconds, connect=30.0)`，其中 `connect=30s` 覆盖 TCP+TLS 握手阶段，`read=timeout_seconds`（最小 600s）覆盖流式数据传输阶段。日志中会同时显示连接超时和流式超时值。

**OpenAI Responses API**：当 `ApiConfig.use_responses_api` 为 `True` 时（由 `.p2p.config` 的 `url_prefix` 匹配或 `--use-responses-api` 显式启用），使用 `client.responses.create()` 替代 `client.chat.completions.create()`。Responses API 使用 `input` 和 `instructions` 替代 `messages`，流式事件格式为 `response.function_call_arguments.delta` 等。工具定义使用 `WRITE_SLIDE_XML_TOOL_RESPONSES`（扁平结构，无 `function` 嵌套）。实现位于 `src/api/openai_responses_client.py`。
...
> **管理方式**：系统提示词以 Markdown 文件形式存放于 `src/prompts/` 目录，运行时通过 `src.prompts.get_system_prompt(lang)` 和 `src.prompts.get_animation_section(lang)` 加载（使用 `functools.cache` 缓存）。工具定义（JSON 结构）保留在 `src/system_prompt.py` 中。

> **语言选择**：与 LLM 交互的所有内容（系统提示词、用户消息）均使用英文，以获得最佳的模型表现。通过 `--prompt-lang zh` 可切换为中文。下方提供英文原文及中文译文。

### 6.1 英文原文（`src/prompts/system_en.md`）
...
7. **Icon Vectorization**: Simple icons and pictograms (checkmarks, gears, light bulbs, user silhouettes, etc.) must be drawn using DrawingML rather than raster placeholders. Approaches in order of preference: preset shapes (`<a:prstGeom>`), Unicode/emoji characters in text boxes, custom geometry (`<a:custGeom>`), or grouped simple shapes.
...
### 6.2 中文译文（`src/prompts/system_zh.md`）
...
7. **图标矢量化**：简单图标（对勾、齿轮、灯泡、用户头像等）必须使用 DrawingML 绘制：预设形状（`<a:prstGeom>`）、Unicode/emoji 文本框、自定义几何（`<a:custGeom>`）或组合形状。仅对高度精细或照片级图标使用光栅占位符。
...
| 用户指令文本 | ~500 tokens |
| **输入总计** | **~25,500 tokens** |
| 每页 Slide XML 输出 | ~9,000 tokens × 20 = ~180,000 tokens |
...
# 从幻灯片截图文件夹转换
p2p ./slide_images/ -o output.pptx
...
p2p input.pdf -o output.pptx \
    ...
    --dpi 192 \                    # LLM 输入图片渲染 DPI（96/144/192/288，默认 192）
    --enable-animations \          # 启用动画 / 转场
    --reasoning-effort medium \    # 推理强度：low/medium/high/xhigh（默认 medium）
    --prompt-lang en \             # 系统提示词语言：en/zh（默认 en）
    --max-pages N \                # 最大转换页数（0=全部，默认 5）
    --pages SPEC \                 # 指定页面，如 0,2,5-8（与 --max-pages 互斥）
    --batch-size 0 \               # 分批大小（0=自动，基于网关超时）
    --output-tps 50 \              # 预估输出 TPS（默认 50）
    --skip-postprocess \           # 跳过光栅图后处理（仅生成占位框）
    --dry-run \                    # Dry-run 模式
    --replay DIR \                 # 从产物目录重放之前的运行
    --continue-run DIR \           # 从产物目录恢复未完成的运行
    -y / --yes \                   # 跳过 TUI 确认，自动执行
    --use-responses-api \          # 使用 OpenAI Responses API（也可由 .p2p.config 自动检测）
...
| 参数 | 环境变量 | 默认值 | 说明 |
|------|----------|--------|------|
| `--api-provider` | `LLM_PROVIDER` | `openai` | API 提供商（openai / anthropic） |
| `--api-base-url` | `OPENAI_BASE_URL` / `ANTHROPIC_BASE_URL` | `""` | API Base URL |
| `--api-key` | `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` | `""` | API Key |
| `--model-name` | `OPENAI_MODEL_NAME` / `ANTHROPIC_MODEL_NAME` | `gpt-5.4` / `claude-opus-4-6` | 模型名称 |
| `--dpi` | — | `192` | LLM 输入图片渲染 DPI（96/144/192/288） |
| `--reasoning-effort` | — | `medium` | 推理强度（low/medium/high/xhigh） |
| `--prompt-lang` | — | `en` | 系统提示词语言（en/zh） |
| `--max-pages` | — | `5` | 最大转换页数（0=全部） |
| `--pages` | — | `""` | 指定页面，如 0,2,5-8（与 --max-pages 互斥） |
| `--batch-size` | — | `0` | 分批大小（0=自动） |
| `--output-tps` | — | `50` | 预估输出 TPS（用于时间估算） |
| `--skip-postprocess` | — | `false` | 跳过光栅图后处理 |
| `--dry-run` | — | `false` | Dry-run 模式 |
| `--replay` | — | `""` | 从产物目录重放 |
| `--continue-run` | — | `""` | 从产物目录恢复未完成运行 |
| `-y`/`--yes` | — | `false` | 跳过 TUI 确认，自动执行 |
| `--use-responses-api` | — | `false` | 使用 OpenAI Responses API（可由 .p2p.config 自动检测） |
| `--log-level` | — | `INFO` | 日志级别（DEBUG/INFO/WARNING/ERROR） |

> **`ApiConfig` 数据类**：`api_key`、`api_base_url`、`model_name` 三个参数封装在 `src.ApiConfig`（frozen dataclass）中，由 `main.py` 统一解析后作为单一参数传递给 `replay.py`、`continue_run.py`、`api/openai_client.py`、`api/anthropic_client.py`，避免多处重复获取环境变量。`ApiConfig` 还包含 `use_responses_api`（bool）和 `extra_headers`（dict），用于 Responses API 和自定义请求头。

> **OpenAI Responses API 与 `.p2p.config`**：当使用 OpenAI 兼容 API 时，可通过 `.p2p.config`（CWD 或 `~/`）配置 `use_responses_api`、`headers`、`headers_from_key`。若 `--api-base-url` 与某条 `providers[].url_prefix` 匹配，则自动应用该配置；也可通过 `--use-responses-api` 显式启用。Responses API 使用 `client.responses.create()` 替代 `client.chat.completions.create()`。

> **Markdown 提示词管理**：系统提示词和动画规则以 Markdown 文件形式存放于 `src/prompts/`，运行时通过 `get_system_prompt(lang)` 和 `get_animation_section(lang)` 加载（使用 `functools.cache` 缓存）。工具定义（JSON 结构）仍保留在 `src/system_prompt.py` 中。

> **TUI 确认**：每次 LLM API 调用前，`src/tui.py` 使用 Rich 面板展示批次信息（模型、Token 数、预估费用、预估时间），用户确认后才执行。通过 `-y`/`--yes` 可跳过确认。
...
│   ├── page_001.png
│   └── ...
├── messages_00.json               # Messages 数组（图片以文件路径引用，非 base64；per-batch，零填充后缀）
├── messages_full_00.json          # 完整 Messages 数组（含 base64，可直接用于 API 调用）
├── tools.json                     # 工具定义
...
对于多批次转换，每批次的产物文件使用零填充后缀（如 `_00`、`_01`）。
...
def estimate_tokens(messages: list[dict], model: str = "gpt-5.4") -> dict:
    ...
    # 输出估算：每页 ~9000 tokens 的 Slide XML + tool call overhead
    estimated_output = image_count * 9000 + 2000
...
│   └── ...
├── messages_00.json               # Messages 数组（图片以文件路径引用）
├── messages_full_00.json          # 完整 Messages 数组（含 base64）
├── tools.json                     # 工具定义
├── system_prompt.md               # 实际使用的系统提示词
├── token_estimate.json            # Token 估算报告
├── api_response_00.json           # API 响应元数据（usage、TPS、耗时等）
├── stream_chunks_00.jsonl         # 原始流式 chunks（JSONL 格式）
├── stream_batch_00.log            # 流式输出日志（实时内容）
├── tool_calls_00.json             # 解析后的 tool call 列表
├── run_params.json                # 本次执行的 CLI 参数
├── reasoning_00.txt               # 模型的思考/推理过程（如有）
└── content_00.txt                 # 模型的非工具调用文本输出（如有）
```

所有批次相关的产物文件均使用零填充后缀命名（如 `messages_00.json`、`api_response_01.json`、`reasoning_02.txt`），即使单批次运行也使用 `_00` 后缀。
...
- **批次内页码**：每个批次在与 LLM 通信时使用批次内部编号（0 到 batch_size-1），而非实际 PDF 页码，以简化 prompt 和工具调用语义
- **零填充文件命名**：所有批次相关的产物文件（如 `messages_00.json`、`api_response_01.json`、`stream_chunks_02.jsonl`、`reasoning_03.txt`）均使用零填充后缀命名，即使单批次运行也使用 `_00` 后缀
- **batch_page_map 重映射**：通过 `batch_page_map` 映射表将 LLM 返回的批次内部页码转换为实际 PDF 页码，应用于 `on_slide_ready` 回调和 `result.slide_xmls` 字典，确保 `slides/` 目录下的 XML 文件使用正确的实际页码命名
...
│   ├── prompts/                   # 系统提示词（Markdown 文件，运行时加载）
│   │   ├── __init__.py            # 提示词加载器（get_system_prompt, get_animation_section）
...
│   ├── message_builder.py         # 构建 OpenAI/Anthropic Messages + Tool 定义
...
│   ├── api_client.py              # 向后兼容重导出 → src.api.openai_client
│   ├── api_client_anthropic.py    # 向后兼容重导出 → src.api.anthropic_client
